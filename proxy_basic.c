...
int main(int argc,char **argv)
{
    ...
    // 프록시가 서버처럼 listen 요청하고 connfd 연다.
...
void doit(int connfd)
{
    ...
    // 클라이언트처럼 서버로 연결하는 descripter를 연다.
    ...
    /*write the http header to endserver의 fd*/
...
void build_http_header(char *http_header, char *hostname, char *path, int port, rio_t *client_rio)
{
    ...
    /*request line */
    // requestlint_hdr_format 함수를 보면 path를 GET %s HTTP/1.0 의 스트링 변수로 받는다.
    sprintf(request_hdr, requestlint_hdr_format, path);
    /*get other request header for client rio and change it */
    while(Rio_readlineb(client_rio, buf, MAXLINE) > 0)
    {
        // \r\n (Enter, 줄바꿈)를 만나면 break
...
void parse_uri(char *uri, char *hostname, char *path, int *port)
{
    *port = 80;
    ...
    if (pos2 != NULL)
    {
        // 문자열을 읽을 때 \0 종료로 인식, 배열 끊어줌
        *pos2 = '\0';
        sscanf(pos, "%s", hostname);
        // '/'가 어떤 정수와 문자열 구분 용도로 사용되어지는 듯하다
