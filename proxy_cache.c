...
typedef struct
{
  ...
  sem_t wmutex;     // 세마코어는 프로세스가 독점사용을 보장하려는, wmutex 쓰레드가 이와 같은
...
int main(int argc, char **argv)
{
  ...
  // 클라이언트로부터 문제가 생겼을 때, 파이프가 끊기면
  // 서버 프로세스 전체가 종료하지 않게 함
...
void doit(int connfd)
{
  ...
  // url_strore와 일치하는 캐시 블록을 찾아서 해당 인덱스를 반환한다.
  // 그리고 그 인덱스로 캐시블록에 있는 데이터를 connfd로 읽게 한다.
...
void readerPre(int i)
{
  // P함수: 접근가능하게함. P와 인자를 체크해서 인자가 비정상이면 에러
  P(&cache.cacheobjs[i].rdcntmutex);
  cache.cacheobjs[i].readCnt++;
  // readCnt가 1이면 사용하고 있다는 표시, 다른 쓰레드 접근을 차단
